...
void send_file(char* file_path, int sock, int file_size)
{
    ...
    send_header(sock, content_type);       // 不发送协议头，直接发送文件
	...
	// 零拷贝
	sendfile(sock, srcfd, NULL, file_size);
	
	/* if (sendall(sock, srcp, &file_size) == -1)// 发送数据
    {
		LOG("line: %d, func: %s, sending file error!\n", __LINE__, __FUNCTION__);
	    return;
    }
